<html>	
	...
	<body>
			...
			<h3>Storing to Safe</h3>
				<div class="row">
					...
					<form action="<?php $_SERVER['PHP_SELF'] ?>" method="post">
						<h4><p>Search: &nbsp <input type="text" name="search" placeholder="Search Document" value = "<?php if (isset($_POST['search'])) echo $_POST['search']; ?>">
						<input type="submit" class="btn btn-primary" value="Enter"></h4>
					</form>
					</div>
					<div class="col-sm-8">
		...
		if (isset($_POST['search'])) {
			...
			if ($count == 0) {
				echo "<script type='text/javascript'>alert('You have inputted an invalid value! Please try again.')</script>";
			...
			else 
			{
				...
				while ($row = mysqli_fetch_array($query))
				{
					...
					$status        =$row['status'];
					$store_safe    =$row['store_safe'];
					$_SESSION['storesafestat']=$store_safe;
					$_SESSION['trackingno']=$tracking;
					$_SESSION ['titleau']=$title;
					
					echo "<tr>
					<td>{$tracking}</div></td>
					
					<td><a href=addrevisionpage2.php>{$title}</a></td>
					<td>{$type}</td>
					...
					<td>{$approvedate}</td>
					<td><a href=storesafe2.php>Store in Safe</a></td>
