<html>
	<head>
		<title>TRACK.IO | View Form</title>
		<meta charset="utf-8">
...
<?php
				...
				if (isset($_POST['view']))
				{
					$formID = $_POST['view'];
					$username = $_SESSION['username'];
					$userquery = mysqli_query($dbc,"SELECT * FROM form_access WHERE username = '{$username}' AND formTrackingNumber ='{$formID}'") or die (mysqli_error($dbc));
					$formquery = mysqli_query($dbc,"SELECT * FROM form_access WHERE formTrackingNumber ='{$formID}' AND username = '{$username}'");
					$formrequestquery =  mysqli_query($dbc,"SELECT * FROM form_access_request WHERE formTrackingNumber  = '{$_POST['view']}' AND username = '{$username}' AND StatusRequest = 'Pending' ") ;//or die (mysqli_query($dbc));
					$formrequestreject = mysqli_query($dbc,"SELECT * FROM form_access_request WHERE formTrackingNumber  = '{$_POST['view']}' AND username = '{$username}' AND StatusRequest = 'Reject' ");
					if(mysqli_num_rows($formrequestreject) > 0)
					{
						echo"
						Sorry, your request to access this form has been denied.
						<form action='ViewForms.php' method='post'>
						<input type='submit' value='View Forms' name='viewForms'>
						</form>";
					}
					else if(mysqli_num_rows($formquery) > 0 OR $_SESSION['usertype'] == '101')
					{
						echo"
						<form action='ViewForms.php' method='post'>
						<input type='submit' value='Back To View Forms' name='viewForms'>
						</form>";
						$formq = mysqli_query($dbc,"SELECT * FROM forms WHERE formTrackingNumber ='{$formID}'");
						echo'
							<table class="table table-hover">
							<thead>
							<tr>	
							<th style="text-align: center;">Form TrackingNumber</th>
							<th style="text-align: center;">Form Title</th>	
							<th style="text-align: center;">Document Tracking ID</th>
							<th style="text-align: center;">Recieve Date by OULC</th>
							<th style="text-align: center;">Recieve Date by DIPO</th>
							</tr>
							</thead><tbody>';
						while ($row = mysqli_fetch_array($formq))
						{
							$formtracking 	=$row['formTrackingNumber'];
							$formtitle 		=$row['formTitle'];
							$formdocument 	=$row['docTrackingNumber'];
							$OULCdate 		=$row['dateFormReceivedOULC'];
							$DIPOdate 		=$row['dateFormReceivedDIPO'];
							echo
							"
							<form action='ViewForms.php' method='post'>
							<tr>
					...
					else if (mysqli_num_rows($formrequestquery) > 0)
					{
						echo
						"
						<p>Your Request to access to this form is still pending.
						<form action='ViewForms.php' method='post'>
						<input type='submit' value='Back To View Forms' name='viewForms'>
						</form>
						"
					...
					{
						echo 
						"<p>You currently don't have access to this form.<br>
						Do you want to request access to this form?<br>
						<form action='ViewForms.php' method='post'>
						<input type='submit' value='Yes' name='yes'>
						<input type='submit' value='No: (Back To View Forms)' name='viewForms'>
				...
				else if (isset($_POST['viewForms']))
				{
					$query = mysqli_query($dbc,"SELECT * FROM forms ");
					echo '
					<table class="table table-hover">
					<thead>
					<tr>	
					...
					while ($row = mysqli_fetch_array($query))
					{
						...
						<tr>
									<td style='text-align:center;'>	
																		<form action='ViewForms.php' method='post'>
																		<input type='submit' value='{$formtracking}' name='view'>
																		</forms>		
																						</td>
									<td>{$formtitle}</td>
						</tr>"; 
				...
				else if (isset($_POST['yes']))
				{
					...
					$query = "INSERT INTO form_access_request (formTrackingNumber, username, StatusRequest)
										VALUES 
										('$formID',
										'$username',
										'Pending')
										";
					mysqli_query($dbc,$query) ;//or die (mysqli_query($dbc));
					echo "<p>Request access to form has been sent.<br>
					<form action='ViewForms.php' method='post'>
					<input type='submit' value='View Forms' name='viewForms'>
					</form>";
				}
