...
$count = 0;
    ...
    if (ISSET($_GET['tracking_no'])){
		...
		$trackingnum = $_GET['tracking_no'];
		$query = "SELECT * FROM documents WHERE trackingNumber LIKE '%".$trackingnum."%' AND  store_safe='1'";
	...
	if ($count == 0) {
		...
		<th style="text-align: center;">Final Comments</th>
	...
	// Code of Text Area where comment is coming from 
	if(isset($_POST['submit'])) {
	
	$comments=$_POST['comments'];

	$sql= "UPDATE documents SET finalComments = '".$comments."' WHERE trackingNumber = ".$_GET['tracking_no'].";";
	$audit ="INSERT into comment_audit (documentTrackingNumber,documentTitle,Comments) values ('$tracking','$title','$comments')";
	$result=mysqli_query($dbc,$sql);
	$auditq = mysqli_query($dbc,$audit);
	echo "Comment submitted";
	header ("Location: comments2.php?tracking_no=".$_GET['tracking_no']);
} 
?>

		<!-- start of commenting code-->
		...
		<!-- form for button that lead to another page -->
		<form action = "<?php echo $_SERVER['PHP_SELF'];?>" method = "post">
		<div class="btn-group">
		<button><a href="comments.php">Search Another File</a></button>
		</div>
		</form>
		</div>
		</div>
