<html>
	...
	<body>
		...
		<div class="container">
			<h3>View Default Path</h3>
			<div class = "row">
				<div class="col-sm-6">
					<a href="pathview.php"><button type="button" class="btn btn-warning btn-md">View Document Paths</button></a>
					<a href="patheditor.php?mode=2"><button type="button" class="btn btn-primary btn-md">Create Document Path</button></a>
				</div>
			</div>
			...
			<table class="table table-hover">
				...
				<tbody>
					<?php
						$query = "SELECT DISTINCT(dpr.pathName) AS 'name', dtr.description, dtr.code
									FROM doc_path_ref dpr
									JOIN doc_type_ref dtr ON dpr.doctype = dtr.code;";
						...
						while ($row = $result->fetch_assoc())
						{
							...
							$findpath = "SELECT dpr.officeCode, dpr.sequence
											FROM doc_path_ref dpr
											JOIN doc_type_ref dtr ON dtr.code = dpr.doctype
											WHERE dpr.pathName = '".$row['name']."' ORDER BY dpr.sequence ASC;";
							...
							{	
								echo "<div class='stepwizard-step'>
											 <button type='button' class='btn btn-default btn-circle btn-disable'>".$path['officeCode']."</button>
										</div>";
							}
							
							echo "</div></div></td>
								<td style='text-align: center; padding-top: 17px;'><span class='label label-warning'><a href='patheditor.php?dt=".$row['description']."&dc=".$row['code']."&mode=3' style='color: white'>Edit Path</a></span> <span class='label label-danger'><a href='#' style='color: white'>Delete Path</a></span></td></tr>";
						}
					?>
				</tbody>
			</table>
