<html>
	<head>
		<title>TRACK.IO | Documents</title>
		<meta charset="utf-8">
	...
	<body>
		...
		else 
		{
				if ($usertype == '102')
				{
					echo "<div class='col-sm-4'><form action='ApproveDocument.php' method='post'>
						<input type= 'submit' value='Approve Document (DIPO)' name='viewapproveDIPO'><br>
						<input type= 'submit' value='Approve Document (OULC)'  name='viewapproveOULC'>
						</forms></div>";
				}
				
				if (isset($_POST['viewapproveDIPO']))
					{
						$query = mysqli_query($dbc,"SELECT * FROM documents WHERE status = 'RFRF'");
						...
						else
						{
							...
							while ($row = mysqli_fetch_array($query))
							{
								...
								$Title 				= $row['title'];
								$RequestingParty 	= $row['requestingPartyID'];
								$Type 				= $row['type'];
								...
								"<tr>
											...
											<td>{$Title}</td>
											<td>{$RequestingParty}</td>
											<td>{$Type}</td>
											<td>{$ProcessingType}</td>
											<td>{$Scope}</td>
											<td>{$Status}</td>	
								<form action='ApproveDocument.php' method='post'>
								<td >
								<input type= 'submit' value='Approve' name='approveDIPO'>
								<input type= 'submit' value='Reject' name='rejectDIPO'>
								<input type='hidden' value='{$TrackingNumber}' name='gettrackingNumber'>
								</div></td>	
								</form>	
								</tr>";
							}
				...
				else if (isset($_POST['viewapproveOULC']))
				{
						$query = mysqli_query($dbc,"SELECT * FROM documents WHERE status = 'ABD'");
						$count = mysqli_num_rows($query);
						if ($count == 0)
							echo "No Pending Documents!";
						else
						{
							...
							while ($row = mysqli_fetch_array($query))
							{
								...
								<form action='ApproveDocument.php' method='post'>
								<td>
								<input type= 'submit' value='Approve' name='approveOULC'>
								<input type= 'submit' value='Reject' name='rejectOULC'>
								<input type='hidden' value='{$TrackingNumber}' name='gettrackingNumber'>
								</div></td>	
								</form>	
								</tr>
				...
				if (isset($_POST['approveOULC']))
				{
					$trackingNumber = $_POST['gettrackingNumber'];
					$query = "UPDATE documents SET status='R' WHERE trackingNumber=$trackingNumber";
					mysqli_query($dbc,$query) or die (mysqli_error($dbc));
					?>
					<script>
					alert('Document has been approved by OULC.');
					</script>
					<?php
					header("Refresh:0");
				}
				if (isset($_POST['rejectOULC']))
				{
					$trackingNumber = $_POST['gettrackingNumber'];
					$query = "UPDATE documents SET status='R' WHERE trackingNumber=$trackingNumber";
					mysqli_query($dbc,$query) or die (mysqli_error($dbc));
				...
				if (isset($_POST['approveDIPO']))
				{
					$trackingNumber = $_POST['gettrackingNumber'];
					$query = "UPDATE documents SET status='AbD' WHERE trackingNumber=$trackingNumber";
					mysqli_query($dbc,$query) or die (mysqli_error($dbc));
					?>
					<script>
					alert('Document has been approved by DIPO.');
					</script>
					<?php
					header("Refresh:0");
				}
				if (isset($_POST['rejectDIPO']))
				{
					$trackingNumber = $_POST['gettrackingNumber'];
					$query = "UPDATE documents SET status='RJ' WHERE trackingNumber=$trackingNumber";
					mysqli_query($dbc,$query) or die (mysqli_error($dbc));
					?>
					<script>
					alert('Document has been rejected.');
