...
if($stat=="s")
    {
        $message='You have not chosen a status! Please <a href="updatedocustatus2.php">select a status</a>';
    }
    else
    {
        session_start();  
        $tn=$_SESSION['documentTrackingNumber'];  
        
        $query=mysqli_query($dbc,"SELECT status FROM documents WHERE trackingNumber='$tn'");
        if($query)
        {
            ...
            $q2=mysqli_query($dbc,"UPDATE documents SET status='$stat' WHERE trackingNumber='$tn'");
            if($q2)
            {
                
                $q3=mysqli_query($dbc,"INSERT INTO status_change_audit (previousStatus,newStatus,documentNumber) VALUES ('$currentstat','$stat','$tn')");
                if($q3)
                {
...
?>
		<div class="container">
			<h3>Updating Document Status</h3>
				...
				<?php echo '<div align="center"><font color="red">'.$message.'</font></div>';?>    
		</div>
	</body>
